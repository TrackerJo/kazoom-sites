...
export function Invitation() {
  ...
  return (
    <div className={`${s.chapterInner} ${s.inviteInner}`}>
      ...
      <div
        key={view}
        className={`${view === "invite" ? s.inviteView : s.workView} ${s.aFade}`}
      >
        {view === "invite" ? (
...
function InviteForm({ onSeeWork }: { onSeeWork: () => void }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!trimmedEmail) next.email = "I need an email to reach you.";
      ...
      next.email = "That email looks off, mind checking it?";
    ...
    return (
      <div className={s.done} role="status" aria-live="polite">
        ...
        <p className={s.inviteSub} tabIndex={-1} ref={doneRef}>
          I will email you at <strong>{email.trim()}</strong> within a day to
          get your site started. No payment yet.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <p
        className={`${s.inviteSub} ${s.aRise}`}
        style={{ animationDelay: "0.4s" }}
      >
        Tell me a little about your business and I will be in touch within a
        day. No payment to start.
      </p>

      <form
        className={`${s.form} ${s.aRise}`}
      ...
      >
        <div className={s.fields}>
          <div className={s.field}>
            <label className={s.label} htmlFor="story-name">
              Business name
            </label>
            <input
              id="story-name"
            ...
            />
            {errors.name && (
              <span className={s.error} id="story-name-error">
                {errors.name}
              </span>
            )}
          </div>
          ...
          <div className={s.field}>
            <label className={s.label} htmlFor="story-email">
              Email
            </label>
            <input
              id="story-email"
            ...
            />
            {errors.email && (
              <span className={s.error} id="story-email-error">
                {errors.email}
              </span>
            )}
          </div>
